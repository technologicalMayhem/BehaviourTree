...
{
    /// <summary>
    /// This class provides extension methods to help editing a <see cref="ConceptualBehaviourTree"/>.
    /// </summary>
    public static class TreeModelHelpers
    {
        ...
        /// </summary>
        /// <param name="tree">The tree model to create the node in.</param>
        ...
        public static NodeModel CreateNode<T>(this ConceptualBehaviourTree tree) => CreateNode(tree, typeof(T));
        ...
        /// <returns>The newly created node.</returns>
        /// <exception cref="ArgumentException">Will be thrown if the node type is not assignable to <see cref="IBehaviourTreeNode"/></exception>
        public static NodeModel CreateNode(this ConceptualBehaviourTree tree, Type type)
        {
        ...
        /// </summary>
        /// <param name="tree">The model the node should be removed from.</param>
        /// <param name="node">The node to be removed from the tree.</param>
        public static void RemoveNode(this ConceptualBehaviourTree tree, NodeModel node) =>
            tree.Nodes.Remove(node);

        /// <summary>
        /// Adds <paramref name="childNode"/> as a parent to <paramref name="parentNode"/>.
        /// </summary>
        /// <param name="tree">The tree model that the relationship should be added to.</param>
        ...
        public static void AddChild(this ConceptualBehaviourTree tree, NodeModel parentNode, NodeModel childNode,
            int parentPortIndex = -1)
        {
            ...
            if (tree.Connections.TryGetValue(parentNode.Id, out var list))
            {
            ...
            {
                tree.Connections[parentNode.Id] = new List<string> { childNode.Id };
            }
        }
        ...
        /// </summary>
        /// <param name="tree">The tree model the relationship is defined in.</param>
        ...
        public static void RemoveParent(ConceptualBehaviourTree tree, NodeModel childNode)
        {
            if (childNode.Type == typeof(RootNode))
                throw new InvalidOperationException("Root node can't be a child to another node");

            var connections = tree.Connections;
        ...
        /// <exception cref="InvalidOperationException">Will be thrown if a root node is passed as <paramref name="childNode"/></exception>
        public static void RemoveChild(ConceptualBehaviourTree tree, NodeModel childNode, NodeModel parentNode)
        {
            if (childNode.Type == typeof(RootNode))
                throw new InvalidOperationException("Root node can't be a child to another node");

            var childrenOfParent = tree.Connections[parentNode.Id];
            childrenOfParent.Remove(childNode.Id);
            if (!childrenOfParent.Any()) tree.Connections.Remove(parentNode.Id);
        }

        /// <summary>
        /// Find a node by it's id.
        /// </summary>
        /// <param name="tree">The tree model to search in.</param>
        ...
        [Pure]
        public static NodeModel GetNodeById(this ConceptualBehaviourTree tree, string id) =>
            tree.Nodes.FirstOrDefault(model => model.Id == id);

        /// <summary>
        /// Get all children of the node.
        /// </summary>
        /// <param name="tree">The tree model containing the relationships.</param>
        /// <param name="parentNode">The node parent node.</param>
        /// <returns>A collection with the children of the parent node.</returns>
        [Pure]
        public static IEnumerable<NodeModel> GetChildren(this ConceptualBehaviourTree tree, NodeModel parentNode)
        {
            var connections = tree.Connections;
            if (connections.ContainsKey(parentNode.Id))
                return connections[parentNode.Id]
                    .Select(s => tree.Nodes.First(model => model.Id == s));
